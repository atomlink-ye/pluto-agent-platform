...
import { QA_GATE_KINDS_V0, RELEASE_READINESS_STATUSES_V0, WAIVER_STATUSES_V0 } from "@/contracts/release.js";
...
describe("event vocabulary compatibility", () => {
  it("keeps the runtime vocabulary aligned with the v1.6 mailbox-and-task-list path", () => {
    ...
    const runStatuses: TeamRunResult["status"][] = ["completed", "completed_with_escalation", "completed_with_warnings", "failed"];
    ...
    expect(agentEventTypes).toContain("mailbox_message");
    expect(agentEventTypes).toContain("task_completed");
    expect(runStatuses).toEqual(["completed", "completed_with_escalation", "completed_with_warnings", "failed"]);
    ...
    expect(DECISION_EVENTS_V0).toEqual(["requested", "commented", "changes_requested", "approved", "rejected", "revoked", "delegated", "expired"]);
