...
async function createAgenticFakePaseoBin(rootDir: string): Promise<string> {
  ...
  const script = [
    ...
    'const toolRoute = (toolName, args) => { if (toolName === "pluto_create_task") return { method: "POST", path: "/tools/create-task", body: args }; if (toolName === "pluto_append_mailbox_message") return { method: "POST", path: "/tools/append-mailbox-message", body: args }; if (toolName === "pluto_complete_run") return { method: "POST", path: "/tools/complete-run", body: args }; throw new Error(`unsupported tool ${toolName}`); };',
    'const callTool = async (state, toolName, args) => { const { url, token, actor } = readInjectedEnv(state); const route = toolRoute(toolName, args); const response = await fetch(`${url}${route.path}`, { method: route.method, headers: { authorization: `Bearer ${token}`, "Pluto-Run-Actor": actor, ...(route.method === "POST" ? { "content-type": "application/json" } : {}) }, ...(route.method === "POST" ? { body: JSON.stringify(route.body) } : {}) }); const text = await response.text(); if (!response.ok) throw new Error(text || `tool call failed for ${toolName}`); return text.length === 0 ? {} : JSON.parse(text); };',
