...
beforeEach(async () => {
  workDir = await mkdtemp(join(tmpdir(), "pluto-bootstrap-reset-local-resume-"));
...
describe("bootstrap reset-local resume", () => {
  it("reconstructs status from underlying stores after local bootstrap records are cleared and resumes without duplicating refs", async () => {
    ...
    await storage.delete("metadata", "workspace-local-v0:local-bootstrap-state");

    const reconciled = await getLocalWorkspaceBootstrapStatus({ dataDir, workspaceId: "workspace-local-v0" });
    expect(reconciled.status).toBe("completed");
    expect(reconciled.session?.createdObjectRefs.map((ref) => ref.id)).toEqual(expect.arrayContaining([
    ...
    const resumed = await getLocalWorkspaceBootstrapStatus({ dataDir, workspaceId: "workspace-local-v0" });
    const refIds = resumed.session?.createdObjectRefs.map((ref) => ref.id) ?? [];
