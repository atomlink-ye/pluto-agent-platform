...
Today Pluto's TypeScript `TeamRunService` is still the real dispatcher:

- Pluto starts a lead session.
- The lead emits text markers like `WORKER_REQUEST: planner :: ...`.
- Pluto parses those markers and starts planner/generator/evaluator sessions itself.
- Pluto collects worker outputs and sends a final `SUMMARIZE` message to the lead.

This proves real agents can run, but it is closer to a marker-based adapter protocol than to a true team-led orchestration model.
...
1. Pluto creates run workspace under the current repo, by default `.tmp/live-quickstart/`, so humans can inspect artifacts locally.
...
- First bounded slice now defines an authored `TeamPlaybookV0` and passes the selected playbook to TeamLead.
- First bounded slice now creates a deterministic file-backed coordination transcript per run and records transcript references in run events/evidence.
- Live/Paseo chat room wiring remains future work; the current transcript is the durable fallback substrate under `.pluto/runs/<runId>/coordination-transcript.jsonl`.
- Default planner → generator → evaluator dependencies are represented in playbook data, and a non-default research-review playbook proves selection is not hard-coded in `TeamRunService` per playbook.
- Full dependency enforcement is still prompt/evidence-level in this slice; generator/evaluator are not yet independently verified against upstream transcript content.
- Evaluator failures currently become blockers; there is no TeamLead-owned revision loop.
- Lead output uses a custom `WORKER_REQUEST` text protocol, so Pluto still owns the operational dispatch.
- Existing live smoke now fails if playbook/transcript evidence is absent, but does not yet prove Paseo chat room use, direct child spawning, dependency content consumption, or evaluator feedback loops.

## First bounded implementation slice — 2026-05-01

Implemented minimal seams toward TeamLead-owned playbook orchestration:
...
- `run_started`, `coordination_transcript_created`, `artifact_created`, `run_completed`, and `evidence.json` carry playbook id, transcript path/room reference, transcript kind, and orchestration source.
...
- Legacy `WORKER_REQUEST:` marker dispatch is labeled fallback-only in prompt/event evidence; it is preserved for compatibility.

Deferred from this bounded slice:

- First-class `paseo chat create/post/read/wait` room wiring.
- Removing Pluto-owned marker dispatch.
- Verifying that generator content actually consumed planner transcript output and evaluator content actually reviewed generator output.
- TeamLead-managed revision loop execution.
...
- Not acceptable as final architecture: Pluto independently decides worker order and dispatches workers from hardcoded marker parsing while TeamLead only emits `WORKER_REQUEST` lines.

## Testing expectations

Keep live test artifacts in the current repository workspace unless explicitly changed:

- Default live workspace: `.tmp/live-quickstart/`
- Do not move Paseo test output to `/Volumes/AgentsWorkspace/tmp/` for this branch.
...
- Live smoke remains inspectable under the current repo `.tmp/live-quickstart/` path.
