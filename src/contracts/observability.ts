...
const OBSERVABILITY_OBJECT_KIND_SET = new Set<string>(OBSERVABILITY_OBJECT_KINDS_V0);
const BUDGET_BEHAVIOR_SET = new Set<string>(BUDGET_BEHAVIORS_V0);
const ALERT_LIFECYCLE_SET = new Set<string>(ALERT_LIFECYCLES_V0);
const RUN_HEALTH_SUMMARY_STATUS_SET = new Set<string>(RUN_HEALTH_SUMMARY_STATUSES_V0);
const OBSERVABILITY_SEVERITY_SET = new Set<string>(OBSERVABILITY_SEVERITIES_V0);
const REDACTION_STATE_SET = new Set<string>(REDACTION_STATES_V0);
...
export function parseBudgetBehaviorV0(value: unknown): BudgetBehaviorLikeV0 | null {
  return parseKnownOrString<BudgetBehaviorV0>(value, BUDGET_BEHAVIOR_SET);
}

export function parseAlertLifecycleV0(value: unknown): AlertLifecycleLikeV0 | null {
  return parseKnownOrString<AlertLifecycleV0>(value, ALERT_LIFECYCLE_SET);
}
...
export function normalizeRunHealthSummaryStatusV0(value: unknown): RunHealthSummaryStatusLikeV0 | null {
  ...
  if (RUN_HEALTH_SUMMARY_STATUS_SET.has(value)) {
    return value as RunHealthSummaryStatusV0;
  }
