...
const PORTABLE_ASSET_KIND_SET = new Set<string>(PORTABLE_ASSET_KINDS_V0);
const PORTABILITY_CONFLICT_RESOLUTION_SET = new Set<string>(PORTABILITY_CONFLICT_RESOLUTIONS_V0);
const PORTABILITY_CONFLICT_OUTCOME_SET = new Set<string>(PORTABILITY_CONFLICT_OUTCOMES_V0);
...
function validateAssets(value: unknown, errors: string[]): void {
  ...
  value.forEach((entry, index) => {
    ...
    errors.push(`assets[${index}].kind must be one of ${PORTABLE_ASSET_KINDS_V0.join(", ")}`);
  });
}
...
function validatePortableAssetKind(value: unknown, path: string, errors: string[]): void {
  ...
  if (!PORTABLE_ASSET_KIND_SET.has(value)) {
    errors.push(`${path} must be one of ${PORTABLE_ASSET_KINDS_V0.join(", ")}`);
  }
}
